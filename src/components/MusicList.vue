<template>
  <div class="music-list-component background"
    ...
    <div class="flex music-list-header">
      <icon-back></icon-back>
      <h3 class="music-list-title font-size-16 text-ellipsis">歌单</h3>
    ...
    <div class="flex music-list-message">
      <div class="music-list-message-cover-container">
        ...
        <div class="music-list-message-listen-count ">
          <span>{{ songListInfo.playCount | transformNumberCount }}</span>
          <i class="music-list-message-listen-icon"></i>
         </div>
    ...
    <div class="flex music-list-operation">
      <div class="operation-collect">
        ...
        <div class="operation-collect-count font-size-10 align-center">{{ songListInfo.subscribedCount }}</div>
      </div>
      <div class="operation-comment">
        <div class="operation-comment-icon background"></div>
        <div class="operation-comment-count font-size-10 align-center">{{ songListInfo.commentCount }}</div>
      </div>
      <div class="operation-share">
        <div class="operation-share-icon background"></div>
        <div class="operation-share-count font-size-10 align-center">{{ songListInfo.shareCount }}</div>
      </div>
    ...
    <div class="music-list-playlist">
      <div class="flex music-list-playlist-header">
        ...
        <h3 class="music-list-playlist-title font-size-16">播放全部<span class="music-list-playlist-count font-size-14">(共{{ songListInfo.trackCount }}首)</span></h3>
            ...
            <div class="flex music-item-container">
              <div class="music-item text-ellipsis">
                <div class="flex music-detail">
                  <p class="music-name font-size-14 text-ellipsis">{{ item.name }}</p>
                  <div v-if="item.mv !== 0" @click.stop="goToMvPlayer(item.mv)" class="icon-mv background"></div>
                </div>
                <div class="music-singer text-ellipsis">{{item.ar | transformArtistList }}{{ ' - ' + item.al.name }}</div>
              </div>
        ...
        if (this.playlist.type !== 'list' || (this.playlist.type === 'list' && this.playlist.id !== this.songListInfo.id)) {
          ...
          this.tracks.forEach(function (value) {
            songList.push({
              ...
              artists: value.ar, //  演唱歌手
              album: value.al, //  专辑信息，主要用于封面图及背景高斯模糊
              mvid: value.mv //  mv链接ID，0为没有id
            })
          })
